...
{
    public class SchoolChangePublish
    {
        public SchoolChangePublish()
        {
        }

        public async Task SendMessageAsync(School school
            , string method)
        {
            string _hostName = "localhost"; // or the RabbitMQ server address
            string _queueName = "School";
            try
            {
                ...
                using (var channel = await connection.CreateChannelAsync())
                {
                    if (connection.IsOpen && channel.IsOpen)
                    {
                        ...
                        var message = JsonSerializer.Serialize(school);
